...
async function checkConsentGate(
  ...
  const gateLeadId =
    (payload._gate_lead_id as string | null) ??
    // Back-compat: existing send-intake-invite callers already pass leadId.
    (payload.leadId as string | null) ?? null;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ─── Gate ──────────────────────────────────────────────────────────────
    const gate = await checkConsentGate(payload, "email", "send-intake-invite");
    if (!gate.allowed) {
