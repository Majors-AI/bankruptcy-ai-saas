...
export default function AttorneyRegistration({ onComplete, onBack }: Props) {
  ...
  const inputCls = (err?: string) =>
    `w-full bg-white border ${err ? 'border-[#DC2626]' : 'border-[#E2E8F0]'} rounded-lg px-3 py-2.5 text-sm text-[#0F172A] placeholder-[#94A3B8] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all`;

  const labelCls = 'block text-xs font-semibold text-[#475569] mb-1 uppercase tracking-wide';
  ...
  function Checkbox({ checked, onChange, children }: { checked: boolean; onChange: () => void; children: React.ReactNode }) {
    return (
      <label className="flex items-start gap-3 cursor-pointer" onClick={onChange}>
        <div className={`flex-shrink-0 w-5 h-5 rounded border-2 mt-0.5 transition-all flex items-center justify-center ${checked ? 'bg-[#2563EB] border-[#2563EB]' : 'border-[#94A3B8] bg-white'}`}>
          {checked && <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg>}
        </div>
        <span className="text-[#475569] text-xs leading-relaxed">{children}</span>
      </label>
    );
  }
  ...
  function NeedsReview() {
    return (
      <div className="text-[10px] text-[#D97706] border border-dashed border-[#D97706]/60 rounded-full px-2.5 py-0.5 inline-block mb-2 uppercase tracking-wider font-semibold">
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-start p-4 pt-8">
        <div className="w-full max-w-lg">
          <button onClick={() => setStep('landing')} className="flex items-center gap-2 text-[#475569] hover:text-[#0F172A] text-xs font-medium mb-6 transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Back
          </button>
          <div className="text-center mb-7">
            <h2 className="text-2xl font-bold text-[#0F172A] tracking-tight">Attorney Account Setup</h2>
            <p className="text-[#475569] text-sm mt-1">Step 1 of 3 — Account Credentials</p>
          </div>

          {serverError && <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-xl p-3 mb-5 text-[#B91C1C] text-sm">{serverError}</div>}
          ...
          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl p-6 space-y-4">
            ...
            <button onClick={handleRegister} disabled={loading} className="w-full bg-[#2563EB] hover:bg-[#3B82F6] disabled:opacity-50 text-white rounded-xl py-3 font-semibold text-sm transition-all mt-2 shadow-lg shadow-[#2563EB]/20">
              {loading ? 'Creating Account...' : 'Continue — Firm Information'}
            </button>
          </div>
          <p className="text-center text-[#475569] text-xs mt-4">
            Already registered?{' '}
            <button onClick={() => setStep('login')} className="text-[#2563EB] hover:text-[#3B82F6] font-medium transition-colors">Sign in</button>
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-7">
            <h2 className="text-2xl font-bold text-[#0F172A] tracking-tight">Attorney Sign In</h2>
            <p className="text-[#475569] text-sm mt-1">bankruptcy.ai — Law Firm Portal</p>
          </div>
          {serverError && <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-xl p-3 mb-5 text-[#B91C1C] text-sm">{serverError}</div>}
          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl p-6 space-y-4">
            <div>
            ...
            <button onClick={handleLogin} disabled={loading} className="w-full bg-[#2563EB] hover:bg-[#3B82F6] disabled:opacity-50 text-white rounded-xl py-3 font-semibold text-sm transition-all shadow-lg shadow-[#2563EB]/20">
        ...
        <div className="w-full max-w-lg">
          <div className="text-center mb-7">
            <div className="flex items-center justify-center gap-2 mb-1">
              {[1,2,3].map(n => (
                <div key={n} className={`h-1.5 w-10 rounded-full transition-all ${n <= 2 ? 'bg-[#2563EB]' : 'bg-[#E2E8F0]'}`} />
              ))}
            </div>
            <h2 className="text-2xl font-bold text-[#0F172A] tracking-tight mt-4">Firm Information</h2>
            <p className="text-[#475569] text-sm mt-1">Step 2 of 3 — Law Firm Details</p>
          </div>
          ...
          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl p-6 space-y-4">
            <div>
              ...
              {errors.firmName && <p className="text-[#DC2626] text-xs mt-1">{errors.firmName}</p>}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                {errors.barNumber && <p className="text-[#DC2626] text-xs mt-1">{errors.barNumber}</p>}
              </div>
              <div>
                ...
                {errors.stateBar && <p className="text-[#DC2626] text-xs mt-1">{errors.stateBar}</p>}
              </div>
            </div>
            <div>
              ...
              {errors.firmAddress && <p className="text-[#DC2626] text-xs mt-1">{errors.firmAddress}</p>}
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                {errors.firmCity && <p className="text-[#DC2626] text-xs mt-1">{errors.firmCity}</p>}
              </div>
              <div>
                ...
                {errors.firmState && <p className="text-[#DC2626] text-xs mt-1">{errors.firmState}</p>}
              </div>
              <div>
                ...
                {errors.firmZip && <p className="text-[#DC2626] text-xs mt-1">{errors.firmZip}</p>}
              </div>
            </div>
            <div>
              <label className={labelCls}>Firm Website <span className="text-[#94A3B8] normal-case font-normal">(optional)</span></label>
            ...
            <button onClick={handleFirmInfoNext} className="w-full bg-[#2563EB] hover:bg-[#3B82F6] text-white rounded-xl py-3 font-semibold text-sm transition-all mt-2 shadow-lg shadow-[#2563EB]/20">
      ...
      <div className="min-h-screen bg-white flex flex-col items-center justify-start p-4 pt-8">
        <div className="w-full max-w-2xl">
          <div className="text-center mb-7">
            <div className="flex items-center justify-center gap-2 mb-1">
              {[1,2,3].map(n => (
                <div key={n} className={`h-1.5 w-10 rounded-full transition-all ${n <= 3 ? 'bg-[#2563EB]' : 'bg-[#E2E8F0]'}`} />
              ))}
            </div>
            <div className="inline-flex items-center gap-2 bg-[#DBEAFE] border border-[#3B82F6]/30 rounded-full px-4 py-1.5 mb-4 mt-4">
              <svg className="w-4 h-4 text-[#2563EB]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
              </svg>
              <span className="text-[#2563EB] text-xs font-semibold tracking-wide uppercase">Third-Party Vendor Disclosures & Consent</span>
            </div>
            <h2 className="text-2xl font-bold text-[#0F172A] tracking-tight">Firm Authorization & Disclosures</h2>
            <p className="text-[#475569] text-sm mt-2 max-w-lg mx-auto">Step 3 of 3 — Review and accept all third-party vendor disclosures on behalf of your law firm.</p>
          </div>

          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl p-6 space-y-5">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-xl p-4">
              <h3 className="text-[#0F172A] font-semibold text-sm mb-2 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-[#DBEAFE] flex items-center justify-center text-[#2563EB] text-xs font-bold flex-shrink-0">1</span>
                Firm Acknowledgments
              </h3>
              <NeedsReview />
              <div className="text-[#475569] text-xs leading-relaxed space-y-2 max-h-72 overflow-y-auto pr-1">
                <p><strong className="text-[#0F172A]">Roles.</strong> The Firm provides the legal services. <strong className="text-[#0F172A]">Bankruptcy.AI, LLC</strong> ("Bankruptcy.AI") provides technology only — Bankruptcy.AI does not practice law and does not give legal advice.</p>
                <p><strong className="text-[#0F172A]">FCRA end user.</strong> For any consumer report obtained through the platform (TransUnion, via <strong className="text-[#0F172A]">iSoftpull</strong>), the Firm is the FCRA "end user," holds the permissible purpose, and is provisioned as its own end-user account. Bankruptcy.AI operates the platform; via the iSoftpull Raw Credit JSON API it receives the report and relays it to the Firm, stores it on the Firm's behalf as processor, and does not use it for its own purposes.</p>
                <p><strong className="text-[#0F172A]">Data ownership / control.</strong> The Firm owns and controls client information (the Firm is the data controller). Bankruptcy.AI does not own client information.</p>
                <p><strong className="text-[#0F172A]">Bankruptcy.AI's role and use of data.</strong> Bankruptcy.AI does not use client information for its own purposes; does not sell, rent, or share it; and does not use it for advertising. Access is limited to operating the platform and supporting the Firm (role-restricted, logged). Bankruptcy.AI may use client information only in <strong className="text-[#0F172A]">de-identified</strong> form to develop, train, and improve its AI models and services where the Firm authorizes and the client has opted in. All client information is encrypted in transit and at rest under SOC 2 controls; each firm's information is segregated from every other firm's.</p>
                <p><strong className="text-[#0F172A]">Firm eligibility, good standing & licensure.</strong> The Firm represents and warrants, at registration and continuously, that it is a law firm or practice in good standing and that each attorney responsible for matters prepared through the platform is licensed and in good standing in the jurisdiction(s) in which the Firm intends to file. The Firm will promptly notify Bankruptcy.AI of any change in this status. Bankruptcy.AI may require evidence as a condition of onboarding and continued access.</p>
                <p><strong className="text-[#0F172A]">Owner identity.</strong> The owner or principal who executes the agreement on the Firm's behalf will submit a copy of a current government-issued photo ID and any other reasonably required proof of identity.</p>
                <p><strong className="text-[#0F172A]">Onboarding sequence.</strong> The Firm's invitation to register and provision accounts is enabled only after the agreement is executed and the owner's identity is verified.</p>
                <p><strong className="text-[#0F172A]">Client consents & firm compliance.</strong> The Firm is responsible for ensuring each client provides the required authorizations before any data is obtained; the platform supplies the capture mechanism. The Firm remains responsible for FCRA, GLBA, Bankruptcy Code §528, applicable state-bar rules and unauthorized-practice-of-law restrictions, and FTC compliance.</p>
                <p><strong className="text-[#0F172A]">Records of agreements.</strong> Bankruptcy.AI retains a copy of the executed agreement, disclosures, acknowledgments, and identity verification in its admin portal as the Firm's service-agreement record. The Firm may request a copy at any time.</p>
                <p><strong className="text-[#0F172A]">Governing agreement.</strong> These acknowledgments are part of and governed by the Firm's MSA + DPA with Bankruptcy.AI, LLC.</p>
              </div>
              <div className="mt-3">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-xl p-4">
              <h3 className="text-[#0F172A] font-semibold text-sm mb-2 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-[#DBEAFE] flex items-center justify-center text-[#2563EB] text-xs font-bold flex-shrink-0">3</span>
                SMS / Voice Communications — Twilio
              </h3>
              <p className="text-[#475569] text-xs leading-relaxed">
                Your firm authorizes bankruptcy.ai to deliver SMS text messages and automated voice notifications to firm staff and, where client consent has been obtained, to clients via <strong className="text-[#0F172A]">Twilio</strong>. For V1, SMS is sent from bankruptcy.ai's master Twilio account. A dedicated {form.firmName || 'firm'} number may be available in V1.1 (see MSA). Firm personnel may opt out of SMS at any time. Message and data rates may apply for staff. Client SMS communications are only sent after client-level consent is captured.
              </p>
              <div className="mt-3">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-xl p-4">
              <h3 className="text-[#0F172A] font-semibold text-sm mb-2 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-[#DBEAFE] flex items-center justify-center text-[#2563EB] text-xs font-bold flex-shrink-0">5</span>
            ...
            <button
              ...
              className="w-full bg-[#2563EB] hover:bg-[#3B82F6] disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl py-3.5 font-semibold text-sm transition-all shadow-lg shadow-[#2563EB]/20"
            >
      ...
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-xl">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[#DBEAFE] border border-[#3B82F6]/30 mb-4 shadow-md shadow-[#2563EB]/10">
              <svg className="w-7 h-7 text-[#2563EB]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.6} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#0F172A] tracking-tight">Request a free demo</h1>
            <p className="text-[#475569] text-sm mt-2 max-w-md mx-auto">
          ...
          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-[#475569] mb-1.5">State of admission *</label>
                <select
                  value={demoForm.stateBar}
                  onChange={e => setDemo('stateBar', e.target.value)}
                  className="w-full bg-white border border-[#E2E8F0] text-[#0F172A] text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all"
                >
                  <option value="">— Select state —</option>
                  {US_STATES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                {fieldErr('stateBar') && <p className="text-xs text-[#DC2626] mt-1">{fieldErr('stateBar')}</p>}
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-[#475569] mb-1.5">Anything we should know? (optional)</label>
              <textarea
                ...
                className="w-full bg-white border border-[#E2E8F0] text-[#0F172A] text-sm rounded-lg px-3 py-2.5 placeholder-[#94A3B8] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] resize-y transition-all"
              />
            </div>

            <p className="text-[11px] text-[#475569] bg-[#DBEAFE] border border-[#3B82F6]/30 rounded-lg px-3 py-2 leading-relaxed">
              <strong className="text-[#0F172A]">Eligibility:</strong> demos are scheduled with law
              firms in good standing. Bar status is verified before the call.
            </p>

            <button
              onClick={submitDemo}
              className="w-full bg-[#2563EB] hover:bg-[#3B82F6] text-white rounded-xl py-3.5 font-semibold text-sm transition-all shadow-lg shadow-[#2563EB]/20 flex items-center justify-center gap-2"
            >
            ...
            <p className="text-center text-xs text-[#475569]">
              ...
              <button onClick={() => setStep('register')} className="text-[#2563EB] hover:text-[#3B82F6] font-semibold">
                Start registration
              </button>
            </p>
          </div>
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="w-full max-w-md text-center">
          <div className="w-20 h-20 rounded-2xl bg-[#ECFDF5] border border-[#16A34A]/30 flex items-center justify-center mx-auto mb-6">
            <svg className="w-10 h-10 text-[#16A34A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/></svg>
          </div>
          <h1 className="text-2xl font-bold text-[#0F172A] tracking-tight">Demo request received</h1>
          <p className="text-[#475569] text-sm mt-3 leading-relaxed">
            Thanks{demoForm.firstName ? `, ${demoForm.firstName}` : ''}. We&apos;ll reach out at{" "}
            <strong className="text-[#0F172A]">{demoForm.email || 'the email you provided'}</strong>{" "}
            within one business day to confirm a time.
          </p>
          <div className="mt-8 flex flex-col gap-2">
            <button
              onClick={() => setStep('register')}
              className="bg-[#2563EB] hover:bg-[#3B82F6] text-white rounded-xl py-3 px-6 font-semibold text-sm transition-colors shadow-lg shadow-[#2563EB]/20"
            >
              Or start registration now
            </button>
            <button
              onClick={() => setStep('landing')}
              className="text-sm text-[#475569] hover:text-[#0F172A] py-2"
            >
              Back to overview
            </button>
          </div>
  ...
  if (step === 'success') {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md text-center">
          <div className="w-20 h-20 rounded-2xl bg-[#DBEAFE] border border-[#3B82F6]/30 flex items-center justify-center mx-auto mb-6 shadow-md shadow-[#2563EB]/10">
            <svg className="w-10 h-10 text-[#2563EB]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
          </div>
          <h2 className="text-2xl font-bold text-[#0F172A] mb-2 tracking-tight">Firm Registration Complete</h2>
          <p className="text-[#475569] text-sm mb-6 leading-relaxed max-w-sm mx-auto">
          ...
          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4 mb-8 text-left space-y-2">
            <p className="text-[#475569] text-xs font-semibold uppercase tracking-wide mb-3">What Happens Next</p>
            {[
              ...
              <div key={item.icon} className="flex gap-3">
                <div className="w-5 h-5 rounded-full bg-[#DBEAFE] flex items-center justify-center text-[#2563EB] text-xs font-bold flex-shrink-0 mt-0.5">{item.icon}</div>
                <p className="text-[#475569] text-xs leading-relaxed">{item.text}</p>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={onComplete}
            className="w-full bg-[#2563EB] hover:bg-[#3B82F6] text-white rounded-xl py-4 font-semibold text-base transition-all shadow-lg shadow-[#2563EB]/20"
          >
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full bg-white border border-[#E2E8F0] text-[#0F172A] text-sm rounded-lg px-3 py-2.5 placeholder-[#94A3B8] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all"
      />
