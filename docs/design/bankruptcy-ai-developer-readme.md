...
4. **Document collection & schedule prep.** The Legal department, with the client (via client portal + Betty), assembles the schedules from the questionnaire and required documents. Document retrieval is **gated on questionnaire completion** (§12) — every applicable question acknowledged (answered or explicitly N/A), with provenance recorded (client / Betty / staff).
...
### Questionnaire completion & answer provenance
Every applicable question must be **acknowledged** before the questionnaire counts as complete — either the client **answers it** or explicitly marks it **does not apply** (N/A / none). A skipped question is *not* the same as N/A; an unaddressed applicable question leaves the questionnaire **incomplete.** Conditional logic is respected — questions that don't apply to this client aren't forced.

Each acknowledgment records **who captured it**: filled out by the **client**, interviewed by **Betty**, or interviewed by **staff.** That provenance travels with the answer and into the time log (§9).

**Document retrieval is gated on completion.** The document checklist is driven by answers as they come in (a Schedule I source selection triggers its document requests, above), but the full document-collection step does not proceed — docs cannot be retrieved — until the questionnaire is complete (every applicable question acknowledged). A partially answered questionnaire shows **"incomplete,"** never a fake-complete status.
